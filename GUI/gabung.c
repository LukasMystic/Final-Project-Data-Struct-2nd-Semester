...
__declspec(dllexport) struct AVLSiswa* search2(struct AVLSiswa *root, char *fullname) {
...
__declspec(dllexport) struct AVLSiswa* deleteDataSiswa(struct AVLSiswa *node, char *fullname) {
    if (node == NULL)
        return node;
